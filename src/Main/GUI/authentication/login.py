...
class LoginScreen(ctk.CTkFrame):
    def __init__(self, master, controller):
        ...
        ctk.CTkEntry(content, width=300, height=40, placeholder_text="Username / Email").pack(pady=10)
        ctk.CTkEntry(content, width=300, height=40, placeholder_text="Password", show="*").pack(pady=10)

        ctk.CTkButton(content, text="Login", width=300, height=40, command=lambda: controller.show_frame("")).pack(pady=20)
        ctk.CTkButton(content, text="Back", width=300, height=40, command=lambda: controller.show_frame("WelcomeScreen")).pack(pady=5)